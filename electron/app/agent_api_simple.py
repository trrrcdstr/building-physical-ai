...
async def process_command(req: CommandRequest):
    ...
    # 钻孔任务（中英文支持）
    if "钻" in cmd or "孔" in cmd or "drill" in cmd:
        import re
        wall_match = re.search(r'([东南西北])墙', cmd)
        wall_key_map = {"东": "east", "西": "west", "南": "south", "北": "north"}
        # 英文方向支持：east wall / north wall / south wall / west wall
        if not wall_match:
            en_match = re.search(r'\b(east|south|north|west)\b\s*wall', cmd)
            wall = en_match.group(1) if en_match else "east"
        else:
            wall = wall_match.group(1)
        wall_key = wall_key_map.get(wall, wall)
    ...
    # 清洁任务（中英文支持）
    elif "清洁" in cmd or "打扫" in cmd or "擦" in cmd or "clean" in cmd or "wash" in cmd or "wipe" in cmd:
        materials = []
    ...
    # 搬运任务（中英文支持）
    elif "搬" in cmd or "移动" in cmd or "move" in cmd or "relocate" in cmd or "carry" in cmd or "lift" in cmd:
        return {
            "task_type": "move",
            "success": True,
            "message": "搬运路径规划完成",
